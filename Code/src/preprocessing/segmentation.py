...
def find_best_mask(img, tol_factor=0.1, search_res=0.1, search_range=(0.2,1)):
    ...
    for i in np.arange(search_range[0], search_range[1]+0.01, search_res):
        ...
        if mask.min() != mask.max():
            corr = dist.correlation(img.flatten(), mask.flatten())# + dist.correlation(img.flatten(), mask.flatten())
        else:
            corr = np.nan
...
img = skimage.io.imread(IN_DATA_PATH + fn.iloc[1437])
...
#               vmin=0, vmax=1, alpha=0.2, zorder=1)
# axs[1].imshow(best_mask, cmap='Greys_r', vmin=0, vmax=1)
# axs[2].plot(np.arange(0.2,1.01,0.1), corr_list, lw=2, color='Orangered')
# axs[2].set_title(f'best : {best_i:.2f}')
# fig.tight_layout()
# plt.show()

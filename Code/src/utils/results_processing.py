...
def metric_barplot(metrics_scores, serie_names, group_names, colors=None, w=None,
                   ...
                   tick_angle=0, legend_kwargs=None):
    """
    ...
    for metric, offset, name, color in zip(metrics_scores, offsets, serie_names, colors):
        ...
        for i, x in enumerate(ind):
            ax.text(x + offset*w/2, means[i]-1.96*stds[i]-0.01, f'{means[i]:.2%}', fontsize=fontsize, ha='center', va='top', rotation=90)
...
def add_stat_significance(pairs, data, serie_names, group_names, w=None, mode='adjusted',
    ...
    stat_display='symbol', avoid_cross=True, link_color='lightgray', text_color='gray', ax=None):
    """
    ...
    for p in pairs:
        ...
        ax.text((x[0]+x[-1])/2, y[1], significance, ha='center', va='bottom', fontsize=fontsize, color=text_color)

...
from src.models.DMSVDD import joint_DMSVDD
...
# Network
nu = 0.01 # fraction of acceptable outlier
ae_pretrain = False
...
def main(seed_i):
    ...
    logger.info(f'{Experiment_Name} soft boundary nu : {nu}')
    logger.info('Network architecture: \n' + summary_string(net, (1, img_size, img_size), device=str(device), batch_size=batch_size) + '\n')

    # initialization of the Model
    jointDMSVDD = joint_DMSVDD(net, nu=nu, soft_boundary=soft_boundary)

    if model_path_to_load:
        jointDMSVDD.load_model(model_path_to_load, map_location=device)
    ...
    logger.info(f'{Experiment_Name} number of warm up epoch: {n_epoch_warm_up}')
    ...
    logger.info(f'{Experiment_Name} criterion weighting : {criterion_weight[0]} Reconstruction loss + {criterion_weight[1]} MSVDD embdedding loss')
    logger.info(f'{Experiment_Name} soft_boundary {soft_boundary} \n')

    # train DMSVDD
    jointDMSVDD.train(train_dataset, lr=lr, n_epoch=n_epoch, n_epoch_pretrain=n_epoch_pretrain, n_epoch_warm_up=n_epoch_warm_up,
                  n_sphere_init=n_sphere_init, lr_milestone=lr_milestone, batch_size=batch_size, weight_decay=weight_decay,
    ...
    # validate DMSVDD
    jointDMSVDD.validate(valid_dataset, device=device, n_jobs_dataloader=n_jobs_dataloader,
                 print_batch_progress=print_batch_progress, criterion_weight=criterion_weight)

    # test DMSVDD
    jointDMSVDD.test(test_dataset, device=device, n_jobs_dataloader=n_jobs_dataloader,
                 print_batch_progress=print_batch_progress, criterion_weight=criterion_weight)

    # save results
    jointDMSVDD.save_results(OUTPUT_PATH + f'results/{Experiment_Name}_results_{seed_i+1}.json')
    logger.info('Test results saved at ' + OUTPUT_PATH + f'results/{Experiment_Name}_results_{seed_i+1}.json' + '\n')

    # save model
    jointDMSVDD.save_model(OUTPUT_PATH + f'model/{Experiment_Name}_model_{seed_i+1}.pt')

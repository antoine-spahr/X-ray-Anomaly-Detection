...
from src.models.JointDeepSVDD import JointDeepSVDD
...
################################################################################
# Import Export
Experiment_Name = 'JointDeepSVDD_Hands'
...
# Network
nu = 0.05 # fraction of acceptable outlier
ae_pretrain = False
...
def main(seed_i):
    ...
    hypersphere center estimation will be more meaningful). Only the hands images
    are used.
    """
    # initialize logger
    ...
    logger.info(f'DeepSVDD soft boundary nu : {nu}')
    logger.info('Network architecture: \n' + summary_string(net, (1, img_size, img_size), device=str(device), batch_size=batch_size) + '\n')

    # initialization of the Model
    jointDSVDD = JointDeepSVDD(net, nu=nu, soft_boundary=soft_boundary, use_subspace=use_subspace)

    if model_path_to_load:
        jointDeepSVDD.load_model(model_path_to_load, map_location=device)
    ...
    logger.info(f'Joint DeepSVDD number of warm up epoch: {n_epoch_warm_up}')
    ...
    logger.info(f'Joint DeepSVDD use subspace {use_subspace}')
    logger.info(f'Joint DeepSVDD soft_boundary {soft_boundary} \n')

    # train DeepSAD
    jointDSVDD.train(train_dataset, lr=lr, n_epoch=n_epoch, n_epoch_pretrain=n_epoch_pretrain, n_epoch_warm_up=n_epoch_warm_up,
    ...
    # validate DeepSAD
    jointDSVDD.validate(valid_dataset, device=device, n_jobs_dataloader=n_jobs_dataloader,
                 print_batch_progress=print_batch_progress, criterion_weight=criterion_weight)

    # test DeepSAD
    jointDSVDD.test(test_dataset, device=device, n_jobs_dataloader=n_jobs_dataloader,
                 print_batch_progress=print_batch_progress, criterion_weight=criterion_weight)

    # save results
    jointDSVDD.save_results(OUTPUT_PATH + f'results/JointDeepSVDD_results_{seed_i+1}.json')
    logger.info('Test results saved at ' + OUTPUT_PATH + f'results/JointDeepSVDD_results_{seed_i+1}.json' + '\n')

    # save model
    jointDSVDD.save_model(OUTPUT_PATH + f'model/JointDeepSVDD_model_{seed_i+1}.pt')
